...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        jarvis = await e.reply(f"𓆩ʙʀᴏᴋᴇɴ𓆪♕︎ʙɪᴛᴇ")
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await jarvis.edit(f"𝐖𝐇𝐀𝐓 𝐈 𝐂𝐀𝐍 𝐃𝐎 𝐅𝐎𝐑 𝐘𝐎𝐔 𝐁𝐀𝐁𝐘 [𝐌𝐲 𝐎𝐖𝐍𝐄𝐑 𝐈𝐒 ⁣༒☠︎𓆩‎ᗪℯ𝓿ꪱ͛ł𓆪☠︎༒ 2.0 ]")
...
async def restart(e):
    if e.sender_id in SUDO_USERS:
        await e.reply(f"` 𝐈 𝐀𝐌 𝐁𝐀𝐂𝐊 𝐒𝐎𝐎𝐍  .`")
        try:
            await X1.disconnect()
        except Exception:
            pass
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"»[⁣༒☠︎𓆩‎ᗪℯ𝓿ꪱ͛ł𓆪☠︎༒ 2.0] 𝐒𝐔𝐃𝐎 𝐃𝐄 𝐃𝐈𝐘𝐀 𝐇𝐄 𝐁𝐀𝐁𝐘  G𝐎 𝐀𝐍𝐃 𝐅𝐔𝐂𝐊 𝐘𝐎𝐔𝐑 s𝐄𝐍𝐄𝐌𝐘'𝐒 😂_")
        target = ""
        ...
        if str(target) in sudousers:
            await ok.edit(f"𓆩ʙʀᴏᴋᴇɴ𓆪♕︎ʙɪᴛᴇ 𝗦𝗨𝗗𝗢 𝗨𝗦𝗘𝗥. !!")
        else:
            if len(sudousers) > 0:
        ...
        await event.reply("» 𝐒𝐎𝐑𝐑𝐘 𝐘𝐀𝐀𝐑 𝐌𝐄𝐑𝐀 𝐎𝐖𝐍𝐄𝐑 𝐇𝐈 𝐒𝐔𝐃𝐎 𝐃𝐄𝐆𝐀 𝐂𝐎𝐍𝐓𝐀𝐂𝐓 𝐓𝐎 [𓆩ʙʀᴏᴋᴇɴ𓆪♕︎ʙɪᴛᴇ]🫧🥀...")
...
async def show_sudo_users(event):
    if event.sender_id == OWNER_ID:
        sudo_users_list = "𓆩ʙʀᴏᴋᴇɴ𓆪♕︎ʙɪᴛᴇ 𝗖𝗨𝗥𝗥𝗘𝗡𝗧 𝗦𝗨𝗗𝗢 𝗨𝗦𝗘𝗥𝗦 𝗟𝗜𝗦𝗧:\n"
        for user_id in SUDO_USERS:
            sudo_users_list += f"- {user_id}\n"
        await event.reply(sudo_users_list)
    else:
        await event.reply("🇴𝗡𝗟𝗬 𝗙𝗢𝗥 ⁣༒☠︎𓆩‎ᗪℯ𝓿ꪱ͛ł𓆪☠︎༒ 2.0 𝗢𝗪𝗡𝗘𝗥.")
